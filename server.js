...
/* -------------------------------------------------------
   CORS
   - On server (prod), you can remove localhost if not needed
-------------------------------------------------------- */
const corsOptions = {
  origin: [
    "http://investordeal.in",
    // "http://localhost:5173", // ⬅️ uncomment for local dev
  ],
  credentials: true,
  optionsSuccessStatus: 200,
  methods: ["GET", "POST", "PUT", "DELETE", "OPTIONS", "PATCH"],
...
/* -------------------------------------------------------
   Static uploads (served by Node)
   If you serve uploads via Nginx alias, this can remain as fallback
-------------------------------------------------------- */
app.use(
  UPLOAD_PUBLIC_BASE,
  helmet.crossOriginResourcePolicy({ policy: "cross-origin" })
...
/* -------------------------------------------------------
   IMPORTANT: Mount specific APIs BEFORE any generic "/api"
   This prevents generic routers from swallowing routes
-------------------------------------------------------- */
app.use("/api/rss-sources", rssRoutes);
...
// If /buy/projects is PUBLIC (non-API), keep it like this.
// If it is part of API, change to: app.use("/api/buy/projects", propertyRoutes);
app.use("/buy/projects", propertyRoutes);

app.use("/api/buyers", require("./routes/buyerRoutes"));

// ⚠️ These two are generic "/api" routers — keep AFTER specific mounts
app.use("/api", require("./routes/buyerTransferRoute"));
app.use("/api", require("./routes/sellerTransferRoute"));
...
app.use("/api/public/system-settings", require("./routes/publicSystemSettings.routes"));
app.use("/api/system-settings", systemSettingsRoutes);
app.use("/api/sms-integration", smsIntegrationRoutes);
app.use("/api/templates", templateRoutes);
app.use("/api/ai", templateContentRoutes);
...
/* -------------------------------------------------------
   Slug redirect (ensure it never touches /api)
   Inside ./middleware/slugRedirect add:
...
app.use("*", (req, res) => {
  res
    .status(404)
    .json({ success: false, message: "Endpoint not found", path: req.originalUrl });
});
