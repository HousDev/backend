...
// ✅ NEW: Send Interactive Message (Buttons, Lists, etc.)
async function sendInteractiveMessage(to, interactiveMessage) {
  ...
  try {
    ...
    // Save to database (optional - can be handled by controller)
    const [contact] = await db.query(
      "SELECT id FROM contacts_wa WHERE phone = ?",
      [to],
    );
    ...
    // Fallback: If interactive message fails, send as text message
...
module.exports = {
  ...
  sendTemplateMessage,
  sendMediaMessage,
  sendInteractiveMessage, // ✅ ADDED - Export the new function
  submitTemplateToMeta,
  getTemplateStatus,
  verifyWebhook,
  handleWebhook,
