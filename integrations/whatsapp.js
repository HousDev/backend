      ...
      for (const change of entry.changes) {
        if (change.field === "messages") {
          ...
          for (const msg of messages) {
            ...
            const contactId = contact[0].id;
            const contactName = contact[0].name || `Customer ${from.slice(-4)}`;
            ...
            if (assignedTo) {
              emitToUser(assignedTo, "new_notification", {
                id: Date.now(),
                title: "New WhatsApp Message",
                body: `${contactName}: ${text.substring(0, 40)}${text.length > 40 ? "..." : ""}`,
                type: "message",
            ...
            // Also emit to contact room for real-time notification
            emitToContactRoom(contactId, "new_notification", {
              id: Date.now(),
              title: "New WhatsApp Message",
              body: `${contactName}: ${text.substring(0, 40)}${text.length > 40 ? "..." : ""}`,
              type: "message",
            ...
            // Emit to assigned user (chat_update)
            if (assignedTo) {
              emitToUser(assignedTo, "chat_update", {
                contact_id: contactId,
                text: text,
