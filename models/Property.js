...
class Property {
  ...
  static async getAll() {
    ...
    SELECT 
      ...
      -- Seller (sellers)
     IFNULL(NULLIF(CONCAT_WS(' ', s.salutation, s.name), ''), p.seller_name) AS seller_name,
  ...
  static async getPopularLocations(limit = 5) {
    const fetchLimit = Math.min(Number(limit) || 5, 100);
    ...
    let rows;
    try {
      [rows] = await db.query(sql, [fetchLimit]);
    } catch (error) {
      [rows] = await db.execute(sql, [fetchLimit]);
    }
