...
const Contact = {
  findAll: async () => {
    try {
      ...
      // Single query to get ALL tags for ALL contacts at once
      ...
      for (const tag of tagRows) {
        if (!tagsByContact[tag.contact_id]) tagsByContact[tag.contact_id] = [];
        tagsByContact[tag.contact_id].push({
          id: tag.id,
          name: tag.name,
          color: tag.color,
        });
      ...
      // Attach tags array to each contact
