...
//   async getAll(conn = null) {
//   // Get all sellers
//   const [sellers] = await runQuery(conn, "SELECT * FROM sellers ORDER BY id DESC");
  
//   // Get properties & co-sellers for each seller
//   for (let seller of sellers) {
//     // properties from my_properties
//     const [properties] = await runQuery(
//       conn, 
//       "SELECT * FROM my_properties WHERE seller_id = ?", 
//       [seller.id]
//     );
//     seller.properties = properties || [];
    
//     // co-sellers from seller_cosellers
//     try {
//       const [coSellers] = await runQuery(
//         conn, 
//         "SELECT * FROM seller_cosellers WHERE seller_id = ?", 
//         [seller.id]
//       );
//       seller.coSellers = coSellers || [];
//     } catch (e) {
//       seller.coSellers = [];
//     }
//   }
...
//     const insertId =
//       res && (res.insertId || (Array.isArray(res) && res[0] && res[0].insertId))
//         ? res.insertId || res[0].insertId
//         : null;
...
//       if (Array.isArray(deleteIds) && deleteIds.length > 0) {
//         await connection.query(
//           `DELETE FROM seller_cosellers WHERE seller_id=? AND id IN (${deleteIds
//             .map(() => "?")
//             .join(",")})`,
//           [id, ...deleteIds]
//         );
...
//              SET salutation=?, name=?, phone=?, whatsapp=?, email=?, relation=?, dob=?, updated_at=CURRENT_TIMESTAMP
//            WHERE id=? AND seller_id=?`,
//           [c.salutation, c.name, c.phone, c.whatsapp, c.email, c.relation, c.dob, Number(c.id), id]
//         );
//       }
