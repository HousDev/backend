...
const blogViewsModel = require("../models/blogViews.model");
...
const recordPostView = async (req, res) => {
  try {
    const postId = isNaN(req.params.id) ? req.params.id : Number(req.params.id);
    ...
    const result = await blogViewsModel.recordView({
      post_id: postId,
      session_id: sessionId,
      ip,
      user_agent: userAgent,
      minutes_window: 24 * 60, // 24 hours
    });

    if (result.inserted) {
      await BlogPost.incrementViews(postId);
    }

    const totals = await blogViewsModel.getPostViews(postId);
...
const likePost = async (req, res) => {
  try {
    ...
    const result = await blogViewsModel.toggleLike({
      post_id: postId,
      session_id: sessionId,
      ip,
      user_agent: userAgent,
    ...
    if (result.liked) {
      await BlogPost.incrementLikes(postId);
    } else {
      await BlogPost.decrementLikes(postId);
    }

    const updated = await BlogPost.findById(postId);
    res.json({ success: true, data: { likes: updated.likes, liked: result.liked } });
  } catch (err) {
