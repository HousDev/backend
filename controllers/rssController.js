...
//     if (!one) return res.status(404).json({ success: false, message: 'Not found' });
...
//     if (!src.active) return res.status(400).json({ success: false, message: 'Source is inactive' });
...
//     }
//     await RSSSourceModel.bumpSyncStats(id, { insertedCount: inserted });
//     await RSSSourceModel.logSync(id, { fetchedCount: fetched, insertedCount: inserted });

//     const latest = await RSSSourceModel.getById(id);
//     res.json({ success: true, data: latest, meta: { fetched, inserted } });
//   } catch (e) {
//     const msg = String(e.message || e);
//     const isParse = /Feed not recognized|Invalid feed URL|HTTP \d+ while fetching feed/i.test(msg);
//     return res.status(isParse ? 422 : 400).json({ success: false, message: msg });
//   }
// };
...
//           if (out.inserted) inserted++;
//         }
//         await RSSSourceModel.bumpSyncStats(src.id, { insertedCount: inserted });
//         await RSSSourceModel.logSync(src.id, {
//           fetchedCount: fetched,
//           insertedCount: inserted,
//         });
//         const latest = await RSSSourceModel.getById(src.id);
//         results.push({ id: src.id, fetched, inserted, source: latest });
//       } catch (err) {
//         await RSSSourceModel.logSync(src.id, {
//           fetchedCount: 0,
//           insertedCount: 0,
...
// // --- Optional utilities (validate/proxy) ---
...
//     const items = Array.isArray(feed.items) ? feed.items.slice(0, Number(limit) || 10) : [];
