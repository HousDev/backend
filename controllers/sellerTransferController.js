...
function parseBudgetToNumber(val) {
  if (val == null) return null;
  const s = String(val).trim().toLowerCase();
  if (s === "") return null;
  if (/(l|lakh|lakhs)$/.test(s)) {
    const m = s.match(/-?[\d,.]+/);
    if (!m) return null;
    const n = parseFloat(m[0].replace(/,/g, ""));
    return isNaN(n) ? null : n / 100; // lakh -> crore
  }
  if (/(cr|crore|crores)$/.test(s)) {
    const m = s.match(/-?[\d,.]+/);
    if (!m) return null;
    const n = parseFloat(m[0].replace(/,/g, ""));
...
// Build property payload: ensures JSON fields are properly stringified and budget numeric
function buildPropertyPayload(overrides, lead, createdBy) {
  const rawBudget = overrides.budget || overrides.budget_range || overrides.price || null;

  const photosJson = ensureJsonString(overrides.photos ?? null);
  const amenitiesJson = ensureJsonString(overrides.amenities ?? null);
  const furnishingJson = ensureJsonString(overrides.furnishing_items ?? overrides.furnishingItems ?? null);
  const nearbyJson = ensureJsonString(overrides.nearby_places ?? overrides.nearbyplaces ?? overrides.nearbyLocations ?? null);
  ...
  return {
    ...
    property_type_name: overrides.property_type ?? overrides.propertyType ?? lead.property_type ?? null,
    property_subtype_name: overrides.property_subtype ?? overrides.property_subtype_name ?? lead.property_subtype ?? null,
    ...
    budget: parseBudgetToNumber(rawBudget),        // numeric in crores
    budget_readable: rawBudget ? String(rawBudget) : null, // keep human string
    ...
    created_at: toSqlDateTime(overrides.created_at) ?? toSqlDateTime(new Date().toISOString()),
    updated_at: toSqlDateTime(overrides.updated_at) ?? toSqlDateTime(new Date().toISOString()),
    lead_id: String(lead.id),
    created_by: asIntOrNull(createdBy) ?? asIntOrNull(overrides.created_by) ?? null,
    updated_by: asIntOrNull(createdBy) ?? asIntOrNull(overrides.updated_by) ?? null,
  };
}
...
function buildSellerPayload(overrides, lead, propertyId, createdBy) {
  return {
    ...
    whatsapp: overrides.whatsapp ?? overrides.whatsapp_number ?? lead.whatsapp_number ?? null,
    ...
    status: overrides.status ?? lead.status ?? null,
    created_by: asIntOrNull(createdBy) ?? asIntOrNull(overrides.created_by) ?? asIntOrNull(lead.created_by) ?? null,
    assigned_to: overrides.assigned_executive ?? overrides.assigned_to ?? lead.assigned_executive ?? null,
    assigned_to_name: overrides.assigned_executive_name ?? overrides.assigned_to_name ?? null,
    lead_id: String(lead.id),
...
async function transferToSeller(req, res) {
  ...
  try {
    ...
    const propCols = Object.keys(propertyPayload).filter(k => propertyPayload[k] !== undefined);
    const propPlaceholders = propCols.map(() => "?").join(", ");
    const propValues = propCols.map(k => propertyPayload[k]);
    const propSql = `INSERT INTO my_properties (${propCols.join(", ")}) VALUES (${propPlaceholders})`;
    const [propInsertRes] = await conn.query(propSql, propValues);

    const propertyId = propInsertRes && (propInsertRes.insertId || (Array.isArray(propInsertRes) && propInsertRes[0] && propInsertRes[0].insertId))
      ? (propInsertRes.insertId || propInsertRes[0].insertId)
      : null;
    if (!propertyId) throw new Error("Failed to insert property");

    const [propRows] = await conn.query("SELECT * FROM my_properties WHERE id = ? LIMIT 1", [propertyId]);
    const createdProperty = Array.isArray(propRows) && propRows.length ? propRows[0] : null;

    // 2) create seller (transactional)
    const sellerPayload = buildSellerPayload(overrides, lead, propertyId, createdBy);

    // dynamic insert using sellerPayload keys (so we won't try to write dropped columns)
    const sellerCols = Object.keys(sellerPayload).filter(k => sellerPayload[k] !== undefined);
    const sellerPlaceholders = sellerCols.map(() => "?").join(", ");
    const sellerValues = sellerCols.map(k => sellerPayload[k]);
    const sellerSql = `INSERT INTO sellers (${sellerCols.join(", ")}) VALUES (${sellerPlaceholders})`;
    const [sellerInsertRes] = await conn.query(sellerSql, sellerValues);

    const createdSellerId = sellerInsertRes && (sellerInsertRes.insertId || (Array.isArray(sellerInsertRes) && sellerInsertRes[0] && sellerInsertRes[0].insertId))
      ? (sellerInsertRes.insertId || sellerInsertRes[0].insertId)
      : null;
    if (!createdSellerId) throw new Error("Failed to insert seller");

    const [sellerRows] = await conn.query("SELECT * FROM sellers WHERE id = ? LIMIT 1", [createdSellerId]);
    const createdSeller = Array.isArray(sellerRows) && sellerRows.length ? sellerRows[0] : null;
    ...
    if (Array.isArray(followups) && followups.length > 0) {
      const followupRowsToInsert = followups.map(fu => {
        ...
        return {
          ...
          schedule_date: scheduleDateTime ? toSqlDateTime(scheduleDateTime) : null,
          schedule_time: scheduleDateTime ? new Date(scheduleDateTime).toTimeString().split(" ")[0] : null,
          ...
          created_at: toSqlDateTime(fu.created_at) ?? toSqlDateTime(new Date().toISOString()),
          updated_at: toSqlDateTime(fu.updated_at) ?? toSqlDateTime(new Date().toISOString()),
        };
      });

      if (followupRowsToInsert.length > 0) {
        // remove columns that are null for all rows to avoid 'unknown column' if table differs
        const cols = Object.keys(followupRowsToInsert[0]).filter(c => followupRowsToInsert.some(r => r[c] !== undefined));
        const placeholders = followupRowsToInsert.map(() => `(${cols.map(() => "?").join(", ")})`).join(", ");
        const values = [];
        followupRowsToInsert.forEach(row => cols.forEach(c => values.push(row[c])));
        const insertSql = `INSERT INTO seller_followups (${cols.join(", ")}) VALUES ${placeholders}`;
        try {
          await conn.query(insertSql, values);
        } catch (e) {
          console.warn("seller_followups insert failed (non-fatal):", e && e.message);
        }
      }
    }
    ...
    return res.status(201).json({
      ...
      transferred_followups_count: Array.isArray(followups) ? followups.length : 0,
    });
  } catch (err) {
    console.error("transferToSeller error:", err && err.message ? err.message : err);
    try {
      if (conn) {
        await conn.rollback();
        conn.release();
